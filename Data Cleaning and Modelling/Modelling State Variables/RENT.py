...
from sklearn.metrics import f1_score, precision_score, recall_score,                             matthews_corrcoef, r2_score, accuracy_score
...
class RENT_Base(ABC):
    ...
    def get_weight_distributions(self, binary = False):
        ...
        for k in self.weight_dict.keys():
            if k[0] == self._best_C and k[1] == self._best_l1_ratio:
                weights_df = weights_df.append(                         pd.DataFrame(self.weight_dict[k]))
    ...
    def plot_object_PCA(self, cl=0, comp1=1, comp2=2, 
        ...
        if problem == "regression" and cl != "continuous":
            sys.exit("The input is invalid. For 'problem = regression', 'cl'                      must be 'continuous' ")
        # catch if classification with continuous. (check if RENT class or RENT reg)

        if cl != 'continuous':
            dat = pd.merge(self.data, self.incorrect_labels.iloc[:,[1,-1]],                                  left_index=True, right_index=True)
            if sel_vars == True:
            ...
            if problem == "regression":
                dat = pd.merge(self.data, self.incorrect_labels.iloc[:,-1],                                          left_index=True, right_index=True)
            else:
                obj_mean = pd.DataFrame(np.nanmean(                         self.get_object_probabilities(), 1),                     index=self.data.index)
                obj_mean.columns = ["pred_means"]
                dat = pd.merge(self.data, obj_mean,                                          left_index=True, right_index=True)
            if sel_vars == True:
                variables = list(self.sel_var)
                variables.extend([-1])
        ...
        if cl != 'continuous':
            ...
            pca_model = ho.nipalsPCA(arrX=data.iloc[:,:-2].values,                                        Xstand=True, cvType=['loo'])
        else:
            pca_model = ho.nipalsPCA(arrX=data.iloc[:,:-1].values,                                        Xstand=True, cvType=['loo'])
        
        scores = pd.DataFrame(pca_model.X_scores())
        scores.index = list(data.index)
        scores.columns = ['PC{0}'.format(x+1) for x in                                  range(pca_model.X_scores().shape[1])]
...
class RENT_Classification(RENT_Base):
    ...
    def __init__(self, data, target, feat_names=[], C=[1,10], l1_ratios = [0.6],
        ...
        if poly == 'ON':
            polynom = PolynomialFeatures(interaction_only=False,                                          include_bias=False)
            self.data = polynom.fit_transform(data)
            polynom_comb = list(combinations_with_replacement(self.feat_names,                                                              2))
            polynom_feat_names = []
...
#                self.train_sets.append(X_train)
                ...
                if(self.method == 'logreg'):
                    self.probas[(C, l1, K)] = pd.DataFrame(                            model.predict_proba(X_test_std), index =                                 X_test.index)
    ...
    def train(self):
        ...
        for C in self.C:
            for l1 in self.l1_ratios:
                count =  0
                vec = pd.DataFrame(np.nan, index= self.indices,                                    columns = ['remove'])
                for k in self.probas.keys():

                    if k[0] == C and k[1] == l1:
                        vec.loc[self.probas[k].index,count] =                         self.probas[k].iloc[:, 1].values
                        count += 1
        ...
        self._scores_df = pd.DataFrame(np.array(means).reshape(                                  len(self.l1_ratios),                                   len(self.C)),         index= self.l1_ratios, columns = self.C)

        self._zeroes_df = pd.DataFrame(index = self.l1_ratios,                                   columns=self.C)
        for l1 in self.l1_ratios:
            for C in self.C:
                count = 0
                for K in range(self.K):
                    nz =                     len(np.where(pd.DataFrame(self.weight_dict[(C, l1, K)])==0)[0])
        ...
        if len(self.C)>1 or len(self.l1_ratios)>1:
            ...
            self._combination = 2 * ((normed_scores.copy().applymap(self.inv) +                                         normed_zeroes.copy().applymap(
                                            self.inv)).applymap(self.inv))
        else:
            self._combination = 2 * ((self._scores_df.copy().applymap(self.inv) +                                  self._zeroes_df.copy().applymap(
                                     self.inv)).applymap(self.inv))
    ...
    def par_selection(self,
            ...
            for reg in C:
                ...
                for train, test in skf.split(self.data, self.target):
                    ...
                    if len(params) == 0:
                        ...
                        zeroes.append((len(self.data.columns)-len(params))                                      /len(self.data.columns))
                        ...
                        scores.append(matthews_corrcoef(test_target,                                         model.predict(test_data_1)))
        ...
        if len(np.unique(scores_df.values)) ==1:
            best_row, best_col = np.where(zeroes_df.values ==                                                   np.nanmax(zeroes_df.values))
            ...
            combination = 2 * ((normed_scores.copy().applymap(self.inv) +                            normed_zeroes.copy().applymap(self.inv)
                           ).applymap(self.inv))
    ...
    def summary_objects(self):
        ...
        self.incorrect_labels = pd.DataFrame({'# test':np.repeat                                      (0, np.shape(self.data)[0]),
                                      'class':self.target,
                                      '# incorrect':np.repeat\
        ...
        self.incorrect_labels['% incorrect'] =         (self.incorrect_labels["# incorrect"]              / self.incorrect_labels['# test']) * 100
    ...
    def get_object_probabilities(self):
        ...
        if self.method != 'logreg':
            ...
            self.pp_data.columns = ['K({0})'.format(x+1)                                         for x in range(
    ...
    def plot_object_probabilities(self, object_id, binning='auto', lower=0,
        ...
        target_objects.index = self.pred_proba_dict[self._best_C,                               self._best_l1_ratio].index
        self.t = target_objects
        for obj in object_id:
            fig, ax = plt.subplots()
            data = self.pred_proba_dict[self._best_C,                               self._best_l1_ratio].loc[obj,:].dropna()
            ...
            ax.set_title('Object: {0}, True class: {1}'.format(obj,                          target_objects.loc[obj,:].values[0]), fontsize=10)
...
class RENT_Regression(RENT_Base):
    ...
    def par_selection(self,
        ...
        self._scores_df_cv, self._zeroes_df_cv, self._combination_cv =             scores_df, zeroes_df, combination
...
#                self.train_sets.append(X_train)
    ...
    def summary_objects(self):
        ...
        self._histogram_data.columns = ['K({0})'.format(x+1)                                         for x in range(
        ...
        self.incorrect_labels.loc[summary_df.index,'# test'] =         summary_df.loc[:,'count']
        self.incorrect_labels.loc[summary_df.index,'average abs error'] =         summary_df.loc[:,'avgerror']

        self.incorrect_labels.iloc[             np.where(self.incorrect_labels.iloc[:,0] == 0)[0]] = np.nan
